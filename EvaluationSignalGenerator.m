function [noised_sig, atom, sig] = EvaluationSignalGenerator(N, coord, SNR)

% -------------------------------------------------------------------------
%   EvaluationSignalGenerator: generates signals as a summmation of atoms using hann window.
%
...
for k = 1:Natoms
    ...
    win = zeros(N, 1);
    win(ceil(t0-T/2+1):ceil(t0+T/2)) = hann(T, 'periodic');
